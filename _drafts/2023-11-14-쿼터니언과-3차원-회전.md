...
기준계 $I$,  좌표계 $A,B$에 대해서, 공간에 고정된 점 $P$를 각각의 좌표계에서 표현한 것을 $P_I, P_A, P_B$라 하자.  

이때, $I$에 상대적으로 $A$가 회전된 정도를  $I$에서 표현하는 회전행렬을 $R_{IA}$, $A\rightarrow B$의 경우에는 $R_{AB}$라 하면, 다음이 성립한다. [벡터 회전과 좌표 변환]({% post_url /수학/회전변환에-관하여/2023-10-24-벡터의-회전과-좌표계-변환의-관계 %}#벡터-회전-vs-좌표계-회전)

$$\begin{align}
R_{IA} P_A  &= P_I \nonumber \\
R_{AB} P_B &= P_A \nonumber \\
\nonumber \\
R_{IA} R_{AB} P_B &= R_{IA} P_A = P_I \nonumber\\
\therefore R_{IB} &= R_{IA}R_{AB}
\end{align}$$

그렇다면, 과연 쿼터니언도 가능할까?

$$\begin{align}
P_I &= q_{IA} \otimes P_A \otimes \overline{q_{IA}} \nonumber \\
P_A &= q_{AB} \otimes P_B \otimes \overline{q_{AB}} \nonumber \\
\nonumber \\
&\left( q_{IA} \otimes q_{AB} \right) \otimes P_B \otimes \overline{\left( q_{IA} \otimes q_{AB} \right)} \nonumber \\
\text{as, } \overline{\left(q_1 \otimes q_2\right)} = \bar{q_2} \otimes \bar{q_1} \nonumber \\
&= \left( q_{IA} \otimes q_{AB} \right) \otimes P_B \otimes \left( \overline{q_{AB}} \otimes \overline{q_{IA}} \right), \nonumber \\
&= q_{IA} \otimes \left( q_{AB} \otimes P_B \otimes \overline{q_{AB}} \right) \otimes \overline{q_{IA}} \nonumber \\
&= q_{IA} \otimes P_A \otimes \overline{q_{IA}} \nonumber \\
&= P_I \nonumber \\
\nonumber \\
\therefore q_{I_B} = q_{IA} \otimes q_{AB}
\end{align}$$

재미있게도, 회전 행렬 처럼 회전의 합성이 동일한 방식으로 가능하다. 

이를 역이용하면, 두 자세간의 차이도 표현할 수 있다.

$$\begin{align}
q_{IA} \otimes q_{AB} &= q_{IB} \nonumber \\
\overline{q_{IA}} \otimes q_{IA} \otimes q_{AB} &= \overline{q_{IA}} \otimes q_{IB} \nonumber \\
\nonumber \\
\therefore q_{AB} = \overline{q_{IA}}\otimes q_{IB}
\end{align}$$

두 자세간의 차이도 자세로 나타난다는게 흥미롭다.  

>쿼터니안으로 표현된 회전의 합성을 정리하면 다음과 같다.  
>사실 뭐 회전행렬이랑 똑같다
>
>$$\begin{align}
>\text{let } q_{A\rightarrow B} &: \text{relative attitude q from } A \text{ to } B \\
>\\
>q_{A\rightarrow C} &= q_{A \rightarrow B} \otimes q_{B \rightarrow C} \\
>q_{B \rightarrow C} &= \overline{q_{A \rightarrow B}} \otimes q_{A \rightarrow C}
>\end{align}$$
{: .prompt-info}

쿼터니언을 사용한 많은 자세 제어 논문에서는 이 방식으로 desired 자세와 현재 자세에 대한 error 자세값을 구한다.  
쿼터니언이 할 수 있는 자세의 자연스러운 전환 및 보간 방식인, [구면 보간](https://en.wikipedia.org/wiki/Slerp)(Spherical linear interpolation, SLERP) 에서도 사용된다.
